...
#define I2C_SLAVE_RX_BUF_LEN 6              /*!< I2C slave rx buffer size */
...
void i2cReceive(void *args) {
  ...
  if (size >= 6) {
    ...
    rollCommandRaw = constrain(((int16_t)data[2] << 8) | data[3], 0, 1000);
    yawCommandRaw = ((int16_t)data[4] << 8) | data[5];

    yawCommandSaturator = map(1000 - yawCommandRaw, 0, 1000, 900, 1100) / 1000.0;
...
#define RMT_TX_GPIO_NORM GPIO_NUM_19       // GPIO pin for DShot output
...
void configureInterrupts() {
  ...
  timerAttachInterrupt(ARM_Cfg, arm_low_ISR);
...
void setup() {
  ...
  conf_slave.slave.slave_addr = ESP_SLAVE_ADDR_1;
...
void loop() {
  ...
  if (currentTime - lastReceived > 50) {
    ...
    sinusoidalThrottle = swThrottle + sin(position * 0.00038366 + phaseDelay) *  absRollCommand * swThrottle;
